...
public sealed class BackgroundRemovalService : IDisposable
{
    ...
    // macOS port: upstream enumerated DXGI adapters to pick the GPU with the most
    // dedicated memory for DirectML. There is no DXGI here, and the CPU provider
    // is the mandatory baseline (PROJECT-SPEC.md section 7), so accelerator
    // selection is resolved once against what the installed runtime actually
    // exposes rather than against a hard-coded provider list.
    private static readonly Lazy<MacAccelerator> PreferredAccelerator = new(FindPreferredAccelerator);
    ...
    private static SessionCreationResult CreateSession(string modelPath, BackgroundRemovalDevice device)
    {
        ...
        {
            return CreateAcceleratedSession(modelPath);
        }
    ...
    /// <summary>
    /// GPU-accelerated session. On macOS the candidate is CoreML.
    ///
    /// PROJECT-SPEC.md section 7 is explicit that CPU operation is mandatory and
    /// that "optional CoreML acceleration only earns parity after model/provider
    /// compatibility and output tolerance checks". Those checks have not been run
    /// on this machine, so CoreML is attempted only when the caller explicitly
    /// asks for GPU, and any failure falls back to CPU rather than erroring.
    /// </summary>
    private static SessionCreationResult CreateAcceleratedSession(string modelPath)
    {
        MacAccelerator accelerator = PreferredAccelerator.Value;

        if (!accelerator.Available)
        {
            throw new NotSupportedException(accelerator.Name);
        }
        ...
        return new SessionCreationResult(new InferenceSession(modelPath, sessionOptions), accelerator.Name);
    }

    /// <summary>
    /// Asks the installed ONNX Runtime which providers it has, instead of
    /// assuming. A build without CoreML reports unavailable and the caller uses
    /// the CPU provider.
    /// </summary>
    private static MacAccelerator FindPreferredAccelerator()
    {
        try
        {
            IReadOnlyCollection<string> providers = OrtEnv.Instance().GetAvailableProviders();

            if (providers.Any(p => p.Contains("CoreML", StringComparison.OrdinalIgnoreCase)))
            {
                return new MacAccelerator(true, "CoreML (output tolerance unverified)");
            }

            return new MacAccelerator(false,
                "CoreML execution provider is not present in this ONNX Runtime build");
        }
        catch (Exception)
        {
            return new MacAccelerator(false, "could not query ONNX Runtime execution providers");
        }
    }
    ...
    /// <summary>Whether an accelerator is usable, and how to describe it.</summary>
    private readonly record struct MacAccelerator(bool Available, string Name);
