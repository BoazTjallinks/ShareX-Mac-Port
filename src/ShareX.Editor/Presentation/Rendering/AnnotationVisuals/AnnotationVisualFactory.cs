...
public static class AnnotationVisualFactory
{
    ...
    private static void RefreshCursorImage(CursorAnnotation cursorAnnotation, Image imageControl)
    {
        ...
        {
            SKBitmap? renderedBitmap = MacCursorBitmapRenderer.CreateAnnotationBitmap(cursorAnnotation.CursorType);
    ...
    private static void RefreshEmojiImage(EmojiAnnotation emojiAnnotation, Image imageControl, bool useInteractiveRender = false)
    {
        ...
        int targetBitmapSize = useInteractiveRender
            ? MacEmojiBitmapRenderer.GetInteractiveStickerSize(renderSize)
            : renderSize;
        ...
        if (needsBitmapRefresh)
        {
            var renderedBitmap = useInteractiveRender
                ? MacEmojiBitmapRenderer.RenderInteractiveStickerBitmap(emojiAnnotation.UnicodeSequence, renderSize)
                : MacEmojiBitmapRenderer.RenderStickerBitmap(emojiAnnotation.UnicodeSequence, renderSize);
                ...
                try
                {
                    ...
                    try
                    {
                        renderedBitmap = await Task.Run(() => useInteractiveRender
                            ? MacEmojiBitmapRenderer.RenderInteractiveStickerBitmap(unicodeSequence, renderSize)
                            : MacEmojiBitmapRenderer.RenderStickerBitmap(unicodeSequence, renderSize));
                        if (renderedBitmap == null)
                        {
                            continue;
                        }
